...
const AuthScreen: React.FC<Props> = ({ onLogin, onAdminLogin }) => {
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      // Legacy Admin Check (Updated pass to meet new rules if needed, but keeping legacy for access)
      // UPDATED: Now accepts '110011' OR 'Password@123'
      ...
      if (mode === 'signup') {
        ...
        // Use Invite Code if exists
        const inviterUid = inviteCode || undefined;
        ...
        if (user.password !== inputPass) throw new Error('Incorrect password.');
        auth.login(user); 
        onLogin(user);
      }
