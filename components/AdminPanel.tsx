...
const AdminPanel: React.FC<Props> = ({ onExit }) => {
  ...
  const loadData = async () => {
      ...
      setStats({
          ...
          totalCoins: allUsers.reduce((acc, u) => acc + (u.typoloBalance || 0), 0)
      });
      const snaps = await localDB.snapshots.getAll();
      setSnapshots(snaps);
      
      const allReports = await localDB.reports.getAll();
      setReports(allReports.sort((a,b) => b.timestamp - a.timestamp));

      const sData = await localDB.stream.get();
      setStreamData(sData || null);
  };

  const handleBan = async (uid: string, currentBan: boolean) => {
      await localDB.users.update(uid, { isBanned: !currentBan });
      loadData();
  };
